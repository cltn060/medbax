...
export function PDFViewer({ url, fileName, isLoading, onClose, className, initialPage = 1 }: PDFViewerProps) {
    ...
    return (
        <div className={cn("flex flex-col h-full bg-slate-50 dark:bg-zinc-900 border-l border-slate-200 dark:border-zinc-800 shadow-xl w-full", className)}>
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 z-10">
                <div className="flex items-center gap-2 overflow-hidden flex-1">
                    ...
                    {isDocumentLoaded && (
                        <span className="text-xs text-slate-400 dark:text-zinc-500 ml-2 whitespace-nowrap">
                            ({pageNumber} / {numPages})
                        </span>
                    )}
                </div>
                ...
                <div className="flex items-center gap-1 ml-2">
                    <div className="flex items-center bg-slate-100 dark:bg-zinc-800 rounded-lg p-0.5 mr-2">
                        <button onClick={zoomOut} className="p-1 text-slate-500 hover:text-slate-700 dark:text-zinc-400 hover:bg-white dark:hover:bg-zinc-700 rounded transition-all" title="Zoom Out">
                            <ZoomOut className="h-4 w-4" />
                        </button>
                        <span className="text-[10px] w-8 text-center text-slate-500 dark:text-zinc-400 selection:bg-none">
                            {Math.round(scale * 100)}%
                        </span>
                        <button onClick={zoomIn} className="p-1 text-slate-500 hover:text-slate-700 dark:text-zinc-400 hover:bg-white dark:hover:bg-zinc-700 rounded transition-all" title="Zoom In">
                    ...
                    {url && (
                        <a
                            ...
                            className="p-1.5 text-slate-500 hover:text-slate-700 dark:text-zinc-400 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
            ...
            <div className="flex-1 relative bg-slate-200/50 dark:bg-black/20 overflow-hidden flex flex-col">
                ...
                {numPages > 1 && (
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg border border-slate-200 dark:border-zinc-700 z-10">
                        <button
                            disabled={pageNumber <= 1}
                            onClick={previousPage}
                            className="p-1 text-slate-600 dark:text-zinc-300 hover:text-indigo-600 dark:hover:text-indigo-400 disabled:opacity-30 transition-colors"
                        >
                            <ChevronLeft className="h-5 w-5" />
                        </button>
                        <span className="text-sm font-medium text-slate-700 dark:text-zinc-200 min-w-[3rem] text-center">
